...
uint32_t unham_24_18(uint32_t a) {
    ...
    uint32_t aa = (a & 0x000004) >> 2 | (a & 0x000070) >> 3 | (a & 0x007f00) >> 4 | (a & 0x7f0000) >> 5;
    //printf ("%s : 0x%x 0x%x 0x%x\n", __func__, aa & 0xff, (aa >> 8) & 0xff, (aa >> 16) & 0xff );
...
void process_telx_packet(data_unit_t data_unit_id, teletext_packet_payload_t *packet, uint64_t timestamp) {
    ...
    if (y == 0) {
        ...
        d_printf ("C11-C14 = 0x%x\n", (unham_8_4(packet->data[7])));
        d_printf ("flag_subtitle = %x\n", flag_subtitle);
        ...
        printf ("hu- pg:%d, m:%d\n", PAGE(config.page), MAGAZINE(config.page));
        ...
        if (page_buffer.tainted == YES) {
            ...
            d_printf ("transmission_mode tainted\n");
        }
    ...
    //
        ...
        for (uint8_t i = 0; i < 40; i++){
            if (page_buffer.text[y][i] == 0x00) 
                page_buffer.text[y][i] = telx_to_ucs2(packet->data[i]);
            //d_printf ("%c %x %x | ", page_buffer.text[y][i], page_buffer.text[y][i], packet->data[i]);
            d_printf ("%c", page_buffer.text[y][i]);
        }
        d_printf ("\n");
        page_buffer.tainted = YES;
    }
        ...
        for (uint8_t j = 0; j < 13; j++) {
            ...
            //d_printf ("Packet 26 : data=0x%x, mode=0x%x , address=0x%x, row_address_group=0x%x\n",data, mode, address, row_address_group);
            ...
            if ((mode == 0x0f) && (row_address_group == NO)) {
                ...
                d_printf ("pkt26 : g2_character ..%x->%c..%x->%c., r:%d, c:%d\n",data , data,
                        page_buffer.text[x26_row][x26_col] , page_buffer.text[x26_row][x26_col],
                        x26_row, x26_col);
            }
            ...
            if ((mode >= 0x11) && (mode <= 0x1f) && (row_address_group == NO)) {
                ...
                d_printf ("pkt26 : g0_diacritical ..%x->%c..%x->%c., r:%d, c:%d\n",data , data,
                        page_buffer.text[x26_row][x26_col] , page_buffer.text[x26_row][x26_col],
                        x26_row, x26_col);
            }
        }
